...
namespace NAK.SimpleAAS
{
    ...
    public class NAKModularSettingsEditor : Editor
    {
        ...
        public ReorderableList GetSettingsList()
        {
            ...
            {
                EditorGUI.LabelField(rect, "Advanced Avatar Settings");
            };
            return list;
        }
        ...
        private void OnDrawAASElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var entity = targetScript.settings[index];
            rect.y += 2;
            rect.x += 12;
            rect.width -= 12;
            ...
            Rect name_textFieldRect = new Rect(foldoutRect.x + foldoutRect.width, rect.y, rect.width - foldoutRect.width, EditorGUIUtility.singleLineHeight);
            ...
            Rect machineName_textFieldRect = new Rect(foldoutRect.x + foldoutRect.width, rect.y, rect.width - foldoutRect.width, EditorGUIUtility.singleLineHeight);
            ...
            float leftOffset = foldoutRect.x + foldoutRect.width;
            ...
            Rect type_enumPopupRect = new Rect(leftOffset, rect.y, (rect.width - leftOffset) / 2, EditorGUIUtility.singleLineHeight);
            var type = (CVRAdvancedSettingsEntry.SettingsType)EditorGUI.EnumPopup(type_enumPopupRect, (SimpleAAS_SettingsType)entity.type);
            if (type != entity.type)
            {
                entity.type = type;
                switch (type)
                {
                    case CVRAdvancedSettingsEntry.SettingsType.GameObjectToggle:
                        ...
                        entity.setting.usedType = GetDefaultType(CVRAdvancedSettingsEntry.SettingsType.GameObjectToggle);
                        break;
                    case CVRAdvancedSettingsEntry.SettingsType.GameObjectDropdown:
                        ...
                        entity.setting.usedType = GetDefaultType(CVRAdvancedSettingsEntry.SettingsType.GameObjectDropdown);
                        break;
            ...
            List<CVRAdvancesAvatarSettingBase.ParameterType> supportedTypes = GetSupportedTypes(entity.type);

            // Create the enum popup rect
            Rect usedType_enumPopupRect = new Rect((rect.width + leftOffset) / 2, rect.y, (rect.width - leftOffset) / 2, EditorGUIUtility.singleLineHeight);

            // Filter the CVRAdvancesAvatarSettingBase.ParameterType enum values to only show the supported types
            List<SimpleAAS_ParameterType> filteredEnum = Enum.GetValues(typeof(CVRAdvancesAvatarSettingBase.ParameterType))
            ...
            if (Enum.IsDefined(typeof(CVRAdvancesAvatarSettingBase.ParameterType), entity.setting.usedType) && supportedTypes.Contains((CVRAdvancesAvatarSettingBase.ParameterType)entity.setting.usedType))
            {
                currentParameterType = (CVRAdvancesAvatarSettingBase.ParameterType)entity.setting.usedType;
            }
            else
            {
            ...
            CVRAdvancesAvatarSettingBase.ParameterType selectedParameterType = (CVRAdvancesAvatarSettingBase.ParameterType)filteredEnum[EditorGUI.Popup(usedType_enumPopupRect, filteredEnum.IndexOf((SimpleAAS_ParameterType)currentParameterType), filteredEnum.Select(x => x.ToString()).ToArray())];

            // Update the setting if the enum value has changed
            if (selectedParameterType != currentParameterType)
            {
                entity.setting.usedType = selectedParameterType;
            }
            ...
            {
                case CVRAdvancedSettingsEntry.SettingsType.GameObjectToggle:
                    var setting_toggle = (CVRAdvancesAvatarSettingGameObjectToggle)entity.setting;
                    DrawToggleField(foldoutRect, ref rect, ref setting_toggle.defaultValue, "Default Value:");
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.GameObjectDropdown:
                    var setting_dropdown = (CVRAdvancesAvatarSettingGameObjectDropdown)entity.setting;
                    DrawDropdownField(foldoutRect, ref rect, ref setting_dropdown.defaultValue, "Default Value:", setting_dropdown.getOptionsList());
                    //draw modified reorderable list
                    rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
                    var options = setting_dropdown.GetReorderableList(null);
                    Initialize_Modified_Dropdown_List(setting_dropdown, options);
                    options.DoList(new Rect(rect.x, rect.y, rect.width, 20f));
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.MaterialColor:
                    var setting_color = (CVRAdvancedAvatarSettingMaterialColor)entity.setting;
                    DrawColorField(foldoutRect, ref rect, ref setting_color.defaultValue, "Default Value:");
                    string helpText_Color = $"Setting Type will use three animator parameters with added suffixes:\n\n{entity.machineName}-r\n{entity.machineName}-g\n{entity.machineName}-b";
                    DrawHelpBox(foldoutRect, ref rect, ref helpText_Color, 4);
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.Slider:
                    var setting_slider = (CVRAdvancesAvatarSettingSlider)entity.setting;
                    DrawSliderField(foldoutRect, ref rect, ref setting_slider.defaultValue, 0f, 1f, "Default Value:");
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.Joystick2D:
                    var setting_inputjoystick2d = (CVRAdvancesAvatarSettingJoystick2D)entity.setting;
                    ...
                    string helpText_Joy2D = $"Setting Type will use two animator parameters with added suffixes:\n\n{entity.machineName}-x\n{entity.machineName}-y";
                    DrawHelpBox(foldoutRect, ref rect, ref helpText_Joy2D, 3);
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.Joystick3D:
                    var setting_inputjoystick3d = (CVRAdvancesAvatarSettingJoystick3D)entity.setting;
                    DrawVector3Field(foldoutRect, ref rect, ref setting_inputjoystick3d.defaultValue, "Default Value:");
                    DrawVector2Field(foldoutRect, ref rect, ref setting_inputjoystick3d.rangeMin, "Range Min:");
                    DrawVector2Field(foldoutRect, ref rect, ref setting_inputjoystick3d.rangeMax, "Range Max:");
                    string helpText_Joy3D = $"Setting Type will use two animator parameters with added suffixes:\n\n{entity.machineName}-x\n{entity.machineName}-y";
                    DrawHelpBox(foldoutRect, ref rect, ref helpText_Joy3D, 3);
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.InputSingle:
                    var setting_inputsingle = (CVRAdvancesAvatarSettingInputSingle)entity.setting;
                    DrawFloatField(foldoutRect, ref rect, ref setting_inputsingle.defaultValue, "Default Value:");
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.InputVector2:
                    var setting_inputvector2 = (CVRAdvancesAvatarSettingInputVector2)entity.setting;
                    DrawVector2Field(foldoutRect, ref rect, ref setting_inputvector2.defaultValue, "Default Value:");
                    string helpText_Input2 = $"Setting Type will use two animator parameters with added suffixes:\n\n{entity.machineName}-x\n{entity.machineName}-y";
                    DrawHelpBox(foldoutRect, ref rect, ref helpText_Input2, 3);
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.InputVector3:
                    var setting_inputvector3 = (CVRAdvancesAvatarSettingInputVector3)entity.setting;
                    DrawVector3Field(foldoutRect, ref rect, ref setting_inputvector3.defaultValue, "Default Value:");
                    string helpText_Input3 = $"Setting Type will use three animator parameters with added suffixes:\n\n{entity.machineName}-x\n{entity.machineName}-y\n{entity.machineName}-z";
                    DrawHelpBox(foldoutRect, ref rect, ref helpText_Input3, 4);
                    break;
            }
        }

        void DrawToggleField(Rect foldoutRect, ref Rect rect, ref bool value, string label)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
            Rect labelRect = new Rect(foldoutRect.x, rect.y, foldoutRect.width * 0.4f, EditorGUIUtility.singleLineHeight);
            float toggleX = labelRect.x + labelRect.width + EditorStyles.label.CalcSize(new GUIContent(label)).x - 20f;
        ...
        void DrawDropdownField(Rect foldoutRect, ref Rect rect, ref int value, string label, string[] options)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
        ...
        void DrawColorField(Rect foldoutRect, ref Rect rect, ref Color value, string label)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
            Rect fieldRect = new Rect(foldoutRect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight);
        ...
        void DrawFloatField(Rect foldoutRect, ref Rect rect, ref float value, string label)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
            Rect fieldRect = new Rect(foldoutRect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight);
        ...
        void DrawSliderField(Rect foldoutRect, ref Rect rect, ref float value, float min, float max, string label)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
            Rect fieldRect = new Rect(foldoutRect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight);
        ...
        void DrawVector2Field(Rect foldoutRect, ref Rect rect, ref Vector2 value, string label)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
            Rect fieldRect = new Rect(foldoutRect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight);
        ...
        void DrawVector3Field(Rect foldoutRect, ref Rect rect, ref Vector3 value, string label)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
            Rect fieldRect = new Rect(foldoutRect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight);
        ...
        void DrawHelpBox(Rect foldoutRect, ref Rect rect, ref string helpText, float lines)
        {
            rect.y += EditorGUIUtility.singleLineHeight * 1.25f;
        ...
        private float OnHeightAASElement(int index)
        {
            ...
            switch (entity.type)
            {
                ...
                case CVRAdvancedSettingsEntry.SettingsType.GameObjectDropdown:
                    {
                        CVRAdvancesAvatarSettingGameObjectDropdown setting_dropdown = (CVRAdvancesAvatarSettingGameObjectDropdown)entity.setting;
                        float height = EditorGUIUtility.singleLineHeight * 1.25f * 7f;
                        float lineHeight = Math.Max((EditorGUIUtility.singleLineHeight * 1.25f * setting_dropdown.options.Count), 20f);
        ...
        private void Initialize_Modified_Dropdown_List(CVRAdvancesAvatarSettingGameObjectDropdown setting_dropdown, ReorderableList dropdown_list)
        {
            ...
            MethodInfo onChangedDropdownListMethod = typeof(NAKModularSettingsEditor).GetMethod("OnChanged_DropdownList", BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            if (onChangedCallbackMethod != onChangedDropdownListMethod)
            {
                ...
                dropdown_list.elementHeightCallback = (int index) =>
                {
                    return EditorGUIUtility.singleLineHeight * 1.25f;
                };
                dropdown_list.drawHeaderCallback = (Rect rect) =>
                {
                    EditorGUI.LabelField(rect, "Dropdown Options");
                };
                dropdown_list.onAddCallback = (ReorderableList list) =>
                {
                    setting_dropdown.options.Add(new CVRAdvancedSettingsDropDownEntry());
                };
                dropdown_list.drawElementCallback = (Rect rect, int index, bool isactive, bool isfocused) =>
                {
                ...
                //dropdown entry name field
                    Rect dropdown_element_textFieldRect = new Rect(dropdown_element_labelFieldRect.x + dropdown_element_labelFieldRect.width, rect.y, rect.width - dropdown_element_labelFieldRect.width, EditorGUIUtility.singleLineHeight);
